  ...
  void _handleLogoTap() {
    ...
    if (_secretTapCount >= 3) {
      ...
      Future.delayed(const Duration(seconds: 1), () {
        if (mounted) Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => const MainDashboard(isDemoMode: true)));
      });
    }
  }

  void _handleLogin() {
    if (mounted) Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => const MainDashboard(isDemoMode: false)));
  }
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  // AI Vector DB Section (Target for Spotlight)
                  Container(
                    padding: const EdgeInsets.all(16),
                    decoration: BoxDecoration(
                      color: Colors.white,
                      borderRadius: BorderRadius.circular(12),
                      boxShadow: [BoxShadow(color: Colors.grey.withAlpha(20), blurRadius: 10)]
                    ),
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            const Text('AI Ghost Scribe', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                          ],
                        ),
                  ...
                  ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(backgroundColor: PulseDesignSystem.darkNavy),
                    child: const Text('Save Record to Private Blockchain'),
                  ),
                  const SizedBox(height: 20),
        ...
        // Zero-Training Onboarding Overlay
        if (_showOnboarding)
          GestureDetector(
              ...
              child: BackdropFilter(
                ...
                child: Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      // Simulated spotlight cutout over Ghost Scribe
                      Container(
                        width: 350,
                        height: 160,
                        decoration: BoxDecoration(
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            const Text("Mulai rekam diagnosis Anda di sini tanpa mengetik.",
                              style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: PulseDesignSystem.bluePrimary)
                            ),
                            const SizedBox(height: 8),
                            const Text("Ketuk dimanapun untuk melanjutkan", style: TextStyle(color: Colors.grey, fontSize: 12)),
                          ],
                        ),
                      )
                    ],
...
class ItCommandModule extends StatelessWidget {
  const ItCommandModule({super.key});

  @override
  Widget build(BuildContext context) {
    return Container(
      color: Colors.black, // Dark aesthetic for IT transparency
      child: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Icon(Icons.terminal, color: Colors.greenAccent, size: 60),
            const SizedBox(height: 20),
            const Text(
              "IT System Health Dashboard",
              style: TextStyle(color: Colors.greenAccent, fontSize: 24, fontFamily: 'monospace', fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 40),
            _buildItMetric("SERVER UPTIME", "99.99%", Colors.greenAccent),
            _buildItMetric("DB LATENCY", "24ms", Colors.greenAccent),
            _buildItMetric("ENCRYPTION", "AES-256-GCM (LOCKED)", Colors.cyanAccent),
            _buildItMetric("NEXUS BRIDGE", "SYNCED (100%)", Colors.greenAccent),
            const SizedBox(height: 40),
            const Text(
              "All systems optimal. Connection secured via enterprise VPC.",
  ...
  Widget _buildItMetric(String label, String value, Color color) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 40.0),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text("> $label", style: const TextStyle(color: Colors.grey, fontSize: 16, fontFamily: 'monospace')),
