/**
 * Server-only Supabase client for the Abecca clinical BFF.
 *
 ...
 * server). Every public table has RLS enabled with no anon policy, so even the
 * publishable key cannot read patient data directly — all access flows through
 * this server BFF.
 */
