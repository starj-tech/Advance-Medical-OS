  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      <p className="text-sm text-muted-foreground">
        Formulary inventory and reorder management.
      </p>

      <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard label="Medications" value={formulary.length} icon={Pill} />
        <StatCard label="Units in Stock" value={formatNumber(totalUnits)} icon={Pill} tone="success" />
        <StatCard
          label="Below Reorder"
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <ul className="divide-y divide-border">
            {formulary.map((m) => {
              const ratio = Math.min(m.stockQuantity / Math.max(m.reorderLevel * 3, 1), 1);
